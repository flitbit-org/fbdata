...
namespace FlitBit.Data
{
	public static class DbConnectionExtensions
	{
		...
		public static DbCommand CreateCommand(this DbConnection connection, string command, CommandType cmdType, int cmdTimeout)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
		...
		public static int ImmediateExecuteNonQuery(this DbConnection connection, Action<DbCommand> preCommand, Action<DbCommand, int> postCommand)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			...
			using (DbCommand cmd = connection.CreateCommand())
			{
				...
				if (postCommand != null) postCommand(cmd, res);
				return res;
			}
		}
		...
		/// <returns>An enumerable over the data reader's rows.</returns>
		public static IEnumerable<IDataRecord> ImmediateExecuteEnumerable(this DbConnection connection, Action<DbCommand> prepareCommand)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
		...
		public static IEnumerable<IDataRecord> ImmediateExecuteEnumerable(this DbConnection connection, string command)
		{
			...
			using (DbCommand cmd = connection.CreateCommand(command))
			using (DbDataReader reader = cmd.ExecuteReader())
			{
				while (reader.Read())
				{
		...
		/// <returns>An enumerable over the DbDataReader rows.</returns>
		public static IEnumerable<IDataRecord> ImmediateExecuteEnumerable(this DbConnection connection, string command, CommandType cmdType)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
			...
			using (DbCommand cmd = connection.CreateCommand(command, cmdType))
			using (DbDataReader reader = cmd.ExecuteReader())
			{
				while (reader.Read())
				{
		...
		/// <returns>An enumerable over the DbDataReader rows.</returns>
		public static IEnumerable<IDataRecord> ImmediateExecuteEnumerable(this DbConnection connection, string command, CommandType cmdType, int cmdTimeout)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
			...
			using (DbCommand cmd = connection.CreateCommand(command, cmdType, cmdTimeout))
			using (DbDataReader reader = cmd.ExecuteReader())
			{
				while (reader.Read())
				{
		...
		public static IEnumerable<T> ImmediateExecuteEnumerable<T>(this DbConnection connection, string command, Func<IDataRecord, T> transform)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
		...
		/// <param name="cmdType">The type of the command being executed: { Text | StoredProcedure | TableDirect }.</param>
		/// <returns>An enumerable over the DbDataReader rows.</returns>
		public static IEnumerable<T> ImmediateExecuteEnumerable<T>(this DbConnection connection, string command, CommandType cmdType, Func<IDataRecord, T> transform)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
		...
		/// <param name="cmdTimeout">the command's timeout.</param>
		/// <returns>An enumerable over the DbDataReader rows.</returns>
		public static IEnumerable<T> ImmediateExecuteEnumerable<T>(this DbConnection connection, string command, CommandType cmdType, int cmdTimeout, Func<IDataRecord, T> transform)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
		...
		public static int ImmediateExecuteNonQuery(this DbConnection connection, string command, CommandType cmdType, int cmdTimeout)
		{
			Contract.Requires<ArgumentNullException>(connection != null);
			Contract.Requires<InvalidOperationException>(connection.State.HasFlag(ConnectionState.Open));
			Contract.Requires<ArgumentNullException>(command != null);
