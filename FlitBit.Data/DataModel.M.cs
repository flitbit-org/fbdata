...
namespace FlitBit.Data
{
	public static class DataModel<M>
	{
					...
					if (typeof(M).IsAbstract)
					{
						...
						if (res == null)
						{
							throw new NotImplementedException(String.Concat("Concrete implementation not found: `",
																															typeof(M).GetReadableFullName(), "`. Are you missing a wireup?"));
						}
						return res;
					}
					return typeof(M);
		...
		static Lazy<Mapping<M>> __mapping = new Lazy<Mapping<M>>(() => Mappings.Instance.ForType<M>(),
																														LazyThreadSafetyMode.ExecutionAndPublication);
		...
		{
			get { return __concreteType.Value; }
		}

		public static Mapping<M> Mapping
		{
			get { return __mapping.Value; }
		}

		public static HierarchyMapping<M> Hierarchy
		{
			get { return __hier.Value; }
		}

		public static IdentityKey<M> IdentityKey
		{
			get {
 				Contract.Ensures(Contract.Result<IdentityKey<M>>() != null);
				return __identityKey.Value; 
			}
		}

		public static IDataModelReferenceFactory<M> ReferenceFactory
		{
			get { return __referenceFactory.Value; }
		}
