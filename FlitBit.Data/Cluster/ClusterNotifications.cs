...
namespace FlitBit.Data.Cluster
{
    ...
    {
        static readonly Lazy<IClusterNotifications> Notifications =
            new Lazy<IClusterNotifications>(() => (FactoryProvider.Factory.CanConstruct<IClusterNotifications>())
        ...
        readonly ConcurrentDictionary<Guid, IClusterObserver> _observers =
            new ConcurrentDictionary<Guid, IClusterObserver>();

        readonly ConcurrentDictionary<string, Channel> _channels = new ConcurrentDictionary<string, Channel>();

        class Channel
        {
            readonly ConcurrentDictionary<string, Observation> _observations =
                new ConcurrentDictionary<string, Observation>();

            public Channel(IClusterObserver observer) { Add(observer); }
            ...
            internal void Notify(ConcurrentDictionary<Guid, IClusterObserver> observers, IClusteredMemory mem, string ch,
                string id, string observation)
            {
                Observation obs;
        ...
        class Observation
        {
            ...
            public Observation(Guid key) { Add(key); }
            ...
            internal void Remove(Guid key)
            {
                while (true)
                {
                    ...
                    var updated = existing.Except(new[]
                    {
                        key
                    }).ToArray();
                    if (updated.Length == 0)
            ...
            public void Notify(ConcurrentDictionary<Guid, IClusterObserver> observers, IClusteredMemory mem, string ch,
                string id, string observation)
            {
                var existing = (Guid[])Volatile.Read(ref this._subscribers);
                if (existing != null)
        ...
        public void Subscribe(IClusterObserver observer)
        {
            ...
            {
                this._channels.AddOrUpdate(observer.Channel, ch => new Channel(observer),
                    (ch, existing) => existing.Add(observer));
            }
        }
