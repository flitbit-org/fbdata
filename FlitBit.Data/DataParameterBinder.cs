...
{
    public class DataParameterBinder : IDataParameterBinder
    {
        ...
        protected virtual IDataParameter MakeParameter(DbCommand command, string name, DbType dbType,
        ...
        protected virtual void PerformSetParameterValue(ParameterBinding[] parameters, int p, string name,
            DateTime value)
        {
            parameters[p].SpecializedValue = value;
        }
        ...
        protected virtual void PerformSetParameterValueAsEnum<E>(ParameterBinding[] parameters, int p, string name,
            E value,
            bool useName)
        {
            if (useName)
        ...
        {
            Contract.Assert(IndexOfParameter(def.Name) < 0,
                "Parameter already exists; parameter bind names must be unique.");
            var binding = new ParameterBinding();
            binding.Definition = def;
            _parameters = _parameters.Concat(new[]
            {
                binding
            })
                                     .ToArray();
        ...
        public bool PrepareDbCommand(DbCommand command)
        {
            ...
            foreach (var binding in parms)
            {
                    ...
                    || def.Size == -1)
                {
                    if (def.Scale > 0)
                    {
                        MakeParameter(command, def.BindName, def.DbType, binding.Definition.SpecializedDbType, def.Size,
                            def.Scale,
                            def.Direction)
                ...
                if (!result
                    && def.Direction.HasFlag(ParameterDirection.Output))
                {
                    result = true;
                }
            }
        ...
        public int GetInt32(DbCommand cmd, string name)
        {
            var idx = IndexOfParameter(name);
            return (int)_parameters[idx].SpecializedValue;
        }
        ...
        public IDataParameterBinder DefineParameter(string name, Type runtimeType, int length,
            ParameterDirection direction)
        {
            var bindName = PrepareParameterName(name);
