...
namespace FlitBit.Data.DataModel
{
    ...
    public static class DataModel<TModel>
    {
        ...
        public static IDataModelBinder Binder { get { return Mapping.GetBinder(); } }
        ...
        public static IDataModelRepository<TModel, TIdentityKey> GetRepository<TIdentityKey>()
        {
            ...
            if (idType == null)
            {
                throw new InvalidOperationException(String.Concat("Data model doesn't define an identity key: ",
                    typeof(TModel).GetReadableSimpleName(), "."));
            }
            if (typeof(TIdentityKey) != IdentityKey.KeyType)
            {
                throw new InvalidOperationException(String.Concat("Invalid identity key type: ",
                    typeof(TIdentityKey).GetReadableSimpleName(), ". The defined identity key type is ",
                    idType.GetReadableSimpleName(), "."));
            }
