...
namespace FlitBit.Data.DataModel
{
  ...
  public abstract class MappedDbTypeEmitter
  {
    ...
    public void DescribeColumn(StringBuilder buffer, ColumnMapping mapping)
    {
      ...
      if (mapping.IsReference
          && mapping.ReferenceTargetMember != null)
      {
    ...
    {
      var details = mapping.DbTypeDetails;
      var bindName = helper.FormatParameterName(overrideBindName ?? details.BindingName);
    ...
    public virtual DbTypeDetails GetDbTypeDetails(ColumnMapping column)
    {
      ...
      var precision = (column.Precision != 0) ? column.Precision : (short?)null;
      var scale = (column.Scale != 0) ? column.Scale : (byte?)null;
      ...
      ColumnMapping<TModel> col, List<string> tableConstraints)
    {
      ...
      if (col.IsReference
          && col.ReferenceTargetMember != null)
      {
    ...
    protected virtual void EmitInvokeDbParameterSetValue(ILGenerator il) { il.CallVirtual<DbParameter>("set_Value"); }

    public virtual void BindParameterOnDbCommand<TDbParameter>(MethodBuilder method, ColumnMapping column,
      string bindingName, LocalBuilder parm, Action<ILGenerator> loadCmd, Action<ILGenerator> loadModel,
      Action<ILGenerator> loadProp,
      LocalBuilder flag)
    ...
    protected virtual string TransformConstantValueToString(object value) { return Convert.ToString(value); }
    ...
    where TDbType : struct
  {
    protected MappedDbTypeEmitter(DbType dbType, TDbType specializedDbType)
      : this(dbType, specializedDbType, typeof(T))
    {}
