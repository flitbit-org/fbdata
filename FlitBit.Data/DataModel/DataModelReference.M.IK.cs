...
namespace FlitBit.Data.DataModel
{
	...
	{
		private static readonly int CHashCodeSeed =
		...
		{
			get { return !EqualityComparer<TModel>.Default.Equals(default(TModel), _model); }
		}
		...
		public TModel Model
		{
			get
			{
				if (!HasModel && HasIdentityKey)
				{
					if (IsFaulted)
					{
						throw new DataModelReferenceException("Unable to resolve reference do to prior fault.", Exception);
					}
					try
					{
		...
		{
			get { return typeof (TIdentityKey); }
		}
