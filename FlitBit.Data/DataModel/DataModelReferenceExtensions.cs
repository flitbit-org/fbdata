...
namespace FlitBit.Data.DataModel
{
	...
	public static class DataModelReferenceExtensions
	{
		...
		public static IDataModelReference<TModel, TIdentityKey> SetIdentityKey<TModel, TIdentityKey>(this IDataModelReference<TModel> reference, TIdentityKey id)
		{
			var referent = reference as IDataModelReferent<TModel, TIdentityKey>;
			if (referent == null)
			{
				if (reference is IDataModelReferent<TModel>)
					throw new InvalidOperationException(String.Concat("Invalid identity key type for ", typeof(TModel).GetReadableSimpleName(), ". Identity key type is ", reference.IdentityKeyType.GetReadableSimpleName(), "."));
				throw new InvalidOperationException(String.Concat("Unable to set referent for references of type: ", reference.GetType().GetReadableFullName(), "."));
			}
		...
		public static IDataModelReference<TModel> SetReferent<TModel>(this IDataModelReference<TModel> reference, TModel model)
		{
