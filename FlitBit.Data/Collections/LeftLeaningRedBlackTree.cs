...
namespace FlitBit.Data.Collections
{
    ...
    public class LeftLeaningRedBlackTree<TKey, TValue>
    {
        ...
        readonly Comparison<TValue> _valueComparison;
        ...
        bool IsMultiDictionary { get { return null != _valueComparison; } }
        ...
        public bool Remove(TKey key)
        {
            if (IsMultiDictionary)
            {
                throw new InvalidOperationException(
                    "Remove is only supported when acting as a normal (non-multi) dictionary.");
            }
        ...
        public IEnumerable<TKey> GetKeys()
        {
            ...
            return Traverse(
                _rootNode,
                n => !lastKeyValid || !Equals(lastKey, n.Key),
                n =>
                {
                    lastKey = n.Key;
                    lastKeyValid = true;
        ...
        public TValue GetValueForKey(TKey key)
        {
            ...
            {
                throw new InvalidOperationException(
                    "GetValueForKey is only supported when acting as a normal (non-multi) dictionary.");
            }
        ...
        Node Add(Node node, TKey key, TValue value)
        {
            if (null == node)
            {
                // Insert new node
            ...
            var comparisonResult = KeyAndValueComparison(key, value, node.Key, node.Value);
        ...
        /// <param name="value">Value to remove.</param>
        /// <returns>True if key/value present and removed.</returns>
        Node Remove(Node node, TKey key, TValue value)
        {
            var comparisonResult = KeyAndValueComparison(key, value, node.Key, node.Value);
            if (comparisonResult < 0)
            {
            ...
            else
            {
                ...
                if (null != node.Right)
                {
                    ...
                    if (0 == KeyAndValueComparison(key, value, node.Key, node.Value))
                    {
                        ...
                        {
                            // Record the removal of the "duplicate" node
                            Debug.Assert(IsMultiDictionary,
                                "Should not have siblings if tree is not a multi-dictionary.");
                            node.Siblings--;
                        }
                        else
                        {
        ...
        Node GetNodeForKey(TKey key)
        {
            ...
            while (null != node)
            {
                ...
                var comparisonResult = _keyComparison(key, node.Key);
                if (comparisonResult < 0)
                {
                    node = node.Left;
                }
        ...
        static T GetExtreme<T>(Node node, Func<Node, Node> successor, Func<Node, T> selector)
        {
            // Initialize
        ...
        IEnumerable<T> Traverse<T>(Node node, Func<Node, bool> condition, Func<Node, T> selector)
        {
            ...
            while (null != current)
            {
                ...
                else
                {
                    do
                    {
                        for (var i = 0; i <= current.Siblings; i++)
                        {
        ...
        int KeyAndValueComparison(TKey leftKey, TValue leftValue, TKey rightKey, TValue rightValue)
        {
            // Compare keys
            var comparisonResult = _keyComparison(leftKey, rightKey);
            if ((0 == comparisonResult)
                && (null != _valueComparison))
            {
        ...
        void AssertInvariants()
        {
            ...
            foreach (var node in Traverse(_rootNode, n => true, n => n))
            {
                if (null != node.Left)
                {
            ...
            var treeCount = -1;
            foreach (var node in Traverse(_rootNode, n => (null == n.Left) || (null == n.Right), n => n))
            {
                var pathCount = 0;
                var current = node;
                while (null != current)
                ...
                Debug.Assert((-1 == treeCount) || (pathCount == treeCount),
                    "Not all paths have the same number of black nodes.");
                treeCount = pathCount;
            }
            ...
            foreach (var node in Traverse(_rootNode, n => true, n => n))
            {
                ...
                if (null != node.Left)
                {
                    Debug.Assert(0 > KeyAndValueComparison(node.Left.Key, node.Left.Value, node.Key, node.Value),
                        "Left node is greater than its parent.");
                }
                // Right node is greater
                if (null != node.Right)
                {
                    Debug.Assert(0 < KeyAndValueComparison(node.Right.Key, node.Right.Value, node.Key, node.Value),
                        "Right node is less than its parent.");
                }
