...
namespace FlitBit.Data.Meta
{
	...
	public sealed class MapColumnAttribute : Attribute
	{
		...
		/// <param name="behaviors">reference behaviors</param>
		/// <param name="referencePropertyNames">the reference target's referenced properties</param>
		public MapColumnAttribute(string targetName, ReferenceBehaviors behaviors, params string[] referencePropertyNames)
		{
			TargetName = targetName;
			ReferenceBehaviors = behaviors;
			References = referencePropertyNames.ToArray();
    ...
    public DbType SuggestedDbType { get; set; }
		...
		{
			ColumnMapping<T> column = mapping.DefineColumn(p);
			if (!String.IsNullOrEmpty(TargetName))
			{
				column.WithTargetName(TargetName);
			}
			ColumnBehaviors behaviors = Behaviors;
			...
			else if (DbTypeTranslator == null && Mappings.ExistsFor(p.PropertyType))
			{
		...
		private void MakeReferenceTo<T>(Mapping<T> mapping, IMapping foreign, PropertyInfo p, ColumnMapping<T> column)
		{
			...
			if (References == null || !References.Any())
			{
				...
				References = new[] {foreignColumn.Member.Name};
			}
			else
			{
