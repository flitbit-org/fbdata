...
namespace FlitBit.Data.Meta
{
        ...
        internal void PrepareMapping<T>(Mapping<T> mapping, Type declaringType)
        {
            ...
            foreach (var p in declaringType.GetProperties())
            {
                ...
                else
                {
                    ...
                    {
                        var meta =
                            (MapInplaceColumnsAttribute)p.GetCustomAttributes(typeof(MapInplaceColumnsAttribute), false)
                                                         .Single();
                    ...
                    {
                        var mapColl =
                            (MapCollectionAttribute)p.GetCustomAttributes(typeof(MapCollectionAttribute), false)
                                                     .Single();
            ...
            foreach (var m in declaringType.GetMethods().Where(m => m.IsDefined(typeof(MapCollectionAttribute), false)))
            {
                mapping.MapCollectionFromMeta(m,
                    (MapCollectionAttribute)m.GetCustomAttributes(typeof(MapCollectionAttribute), false)
                                             .Single());
            }
        }
    ...
    internal static class EntityWireupObserver
    {
        ...
        static readonly MethodInfo RegisterMethod = typeof(IFactory).MatchGenericMethod(
