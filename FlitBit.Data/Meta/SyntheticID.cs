...
namespace FlitBit.Data.Meta
{
	public struct SyntheticID : IEquatable<SyntheticID>
	{
		...
		/// <returns><em>true</em> if this and the other are equal; otherwise <em>false</em></returns>
		...
		/// <returns><em>true</em> if the value is formatted as a valid identity; otherwise <em>false</em></returns>
		...
		public static bool IsValidID(char[] value)
		{
			...
			{        
				...
				else return false; // input contains a non-digit character
			}
			return (sum % 0xF == 0);
		}
		...
		public static char CalculateCheckDigit(char[] value)
		{
			...
			{
				var digit = (value[len - i] - AsciiOffsetToDigitZero);
				if (digit < 0) throw new ArgumentException();
