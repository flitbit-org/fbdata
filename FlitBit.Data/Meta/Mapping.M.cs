...
namespace FlitBit.Data.Meta
{
  ...
  internal class Mapping<TModel> : IMapping<TModel>
  {
    ...
    public IMapping<TModel> ReferencesType<TOther>(Expression<Func<TModel, TOther, bool>> expression) { return this; }
                                       ...
                                       }))
      {
        if (it.IsDefined(typeof(MapEntityAttribute), false))
        {
          if (it != typeof(TModel))
          {
            ...
            foreach (
              var idcol in
                baseMapping.Identity.Columns.Where(c => c.Column.Member.DeclaringType == baseMapping.RuntimeType))
            {
