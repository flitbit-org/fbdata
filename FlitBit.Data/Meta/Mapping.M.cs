...
namespace FlitBit.Data.Meta
{
    ...
    internal class Mapping<TModel> : IMapping<TModel>
    {
        ...
        public MemberInfo InferCollectionReferenceTargetMember(MemberInfo member, IMapping elmMapping)
        {
            ...
            {
                throw new MappingException(String.Concat("Relationship not defined between ", typeof(TModel).Name, ".",
                    member.Name,
        ...
        public IMapping<TModel> MapAllOperations() { return MapAllOperations(this.Strategy); }
        ...
        public IMapping<TModel> MapAllOperations(MappingStrategy strategy)
        {
            ...
            {
                warnings.Add(
                    "TargetCatalog has not been configured; the catalog will be determined by the connection string.");
            }
            ...
            {
                warnings.Add(String.Concat("TargetObject has not been configured; the type name will be used: ",
                    typeof(TModel).Name));
            }
        ...
        public CollectionMapping<TModel> DefineCollection(MemberInfo member, string name)
        {
            ...
            {
                throw new MappingException(String.Concat("Collection name defined on method ", member.Name,
                    " conflicts with an existing property's name: ", name, "."));
            }
                                             ...
                                             }))
            {
                if (it.IsDefined(typeof(MapEntityAttribute), false))
                {
                    if (it != typeof(TModel))
                    {
                        ...
                        foreach (
                            var idcol in
                                baseMapping.Identity.Columns.Where(
                                    c => c.Column.Member.DeclaringType == baseMapping.RuntimeType))
                        {
        ...
        {
            Contract.Requires<ArgumentException>(member.MemberType == MemberTypes.Property
                                                 || member.MemberType == MemberTypes.Method);
            var type = (member.MemberType == MemberTypes.Property)
            ...
            {
                throw new MappingException(String.Concat("Unable to fulfill collection mapping on ", typeof(TModel).Name,
                    ".",
                    member.Name,
            ...
            {
                throw new MappingException(String.Concat("The mapped collection on ", typeof(TModel).Name, ".",
                    member.Name,
            ...
            else
            {
                ...
                {
                    throw new MappingException(String.Concat("Unable to fulfill collection mapping on ",
                        typeof(TModel).Name, ".",
                        member.Name,
        ...
        public void MapColumnFromMeta(PropertyInfo p, MapColumnAttribute mapColumn)
        {
            ...
            if (Mappings.ExistsFor(p.PropertyType))
            {
                ...
                if (mapColumn.References == null
                    || !mapColumn.References.Any())
                {
                    foreignColumn = foreignMapping.GetPreferredReferenceColumn();
                    if (foreignColumn == null)
                    {
                        throw new MappingException(String.Concat("Relationship not defined between ",
                            typeof(TModel).Name, ".",
                            p.Name,
                            " and the referenced type: ", p.PropertyType.Name));
                    }

                    mapColumn.References = new[]
                    {
                ...
                else
                {
                    ...
                    foreignColumn =
                        foreignMapping.Columns.FirstOrDefault(c => c.Member.Name == mapColumn.References.First());
                }
        ...
        public string ConnectionName
        {
            get
            {
                return String.IsNullOrEmpty(_connectionName) ? Mappings.Instance.DefaultConnection : _connectionName;
            }
        ...
        public IDataModelBinder GetBinder()
        {
            ...
            _binder = (IDataModelBinder)get.Invoke(helper, new object[]
            {
                this
            });
            return _binder;
        ...
        {
            var ht = typeof(IHierarchyMapping<TModel>).MatchGenericMethod("NotifySubtype", 1, typeof(void),
                typeof(IMapping<>))
                                                      .MakeGenericMethod(mapping.RuntimeType);
            ht.Invoke(Hierarchy, new object[]
            {
                mapping
            });
        }
