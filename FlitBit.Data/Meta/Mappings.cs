...
namespace FlitBit.Data.Meta
{
	...
	public sealed class Mappings
	{
		...
		private static readonly Lazy<Mappings> Singleton = new Lazy<Mappings>(() => new Mappings(),
			LazyThreadSafetyMode.ExecutionAndPublication);

		private readonly Dictionary<object, IMapping> _mappings = new Dictionary<object, IMapping>();
		...
		{
			object key = typeof (IMapping<TModel>).GetKeyForType();
			Mapping<TModel> res;
			bool mustInit = false;
		...
		internal static bool ExistsFor(Type type)
		{
			if (Type.GetTypeCode(type) == TypeCode.Object && !type.IsValueType)
			{
				...
				string thisAssemblyName = Assembly.GetExecutingAssembly()
					.GetName()
					.FullName;
