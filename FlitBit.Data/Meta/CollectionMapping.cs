        ...
        Type CalculateColumnTypeFromJoinProperties(MemberInfo local, MemberInfo reference)
        {
            ...
            if (lcolumn == null)
            {
                throw new MappingException(String.Concat("The mapped collection on ", LocalMapping.RuntimeType.Name, ".",
                    LocalMember.Name,
                    " uses a local property that is not mapped to a column: ", LocalMapping.RuntimeType.Name, ".",
                    local.Name)
                    );
            }
            ...
            if (lcolumn.UnderlyingType != rcolumn.UnderlyingType)
            {
                throw new MappingException(String.Concat("The mapped collection on ", LocalMapping.RuntimeType.Name, ".",
                    LocalMember.Name,
                    " references incompatible properties: ", LocalMapping.RuntimeType.Name, ".", local.Name,
                    " cannot equal ",
