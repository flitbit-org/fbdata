...
namespace FlitBit.Data.SqlServer
{
        ...
        where TModelImpl : class, TDataModel, IDataModel, new()
    {
            ...
            if (!members.Contains(mapping.RuntimeType))
            {
                ...
                if (string.IsNullOrEmpty(mapping.ConnectionName))
                {
                    throw new MappingException(
                        "ConnectionName must be set before creating SQL commands for a data model.");
                }
                ...
                foreach (
                    var dep in
                        mapping.Dependencies.Where(
                            d =>
                            d.Kind == DependencyKind.Base
                            || d.Kind.HasFlag(DependencyKind.Direct) && d.Target != mapping))
                {
        ...
        {
            var builder =
                new DataModelQueryBuilder<TDataModel, TIdentityKey, SqlConnection>(
                    (IDataModelRepository<TDataModel, TIdentityKey, SqlConnection>)repository, LegacyWriter);
