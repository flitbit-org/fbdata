	...
	{														
		public static ColumnMapping GetPreferredReferenceColumn(this IMapping mapping)
		{
			if (mapping.IsEnum)
			{
				...
				if (idcol == null)
					throw new MappingException(String.Concat("Entity type '", mapping.RuntimeType.GetReadableFullName(), "' declares behavior EntityBehaviors.MapEnum but the enum type cannot be determined. Specify an identity column of enum type."));
				var namecol = mapping.Columns.Where(c => c.RuntimeType == typeof(String) && c.IsAlternateKey).FirstOrDefault();
				if (namecol == null)
					throw new MappingException(String.Concat("Entity type '", mapping.RuntimeType.GetReadableFullName(), "' declares behavior EntityBehaviors.MapEnum but a column to hold the enum name cannot be determined. Specify a string column with alternate key behavior."));
				var mapEnum = (MapEnumAttribute)idcol.RuntimeType.GetCustomAttributes(typeof(MapEnumAttribute), false).SingleOrDefault();
		...
		public static DDLBatch GetDdlBatch(this IMapping mapping, DDLBatch batch, Stack<IMapping> stack)
		{
			...
			finally
			{
				...
				if (!Object.ReferenceEquals(bottom, mapping))
					LogSink.OnTraceEvent(TraceEventType.Warning,
						String.Concat("GetDdlBatch encountered an error in the stack while generating DDL; stack contained {0} instead of {1} on exit.",
		...
		{
			Contract.Requires<ArgumentNullException>(mapping != null);
			Contract.Requires<InvalidOperationException>(mapping.ConnectionName != null && mapping.ConnectionName.Length > 0, "ConnectionName must be set before creating SQL commands for an entity");
			Contract.Ensures(Contract.Result<DDLBatch>() != null);
