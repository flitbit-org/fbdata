    ...
    SqlValueExpression InferJoinForMember(SqlMemberAccessExpression item, string path)
    {
      if (item.Join == null)
      {
        ...
        if (join == null)
        {
          ...
          join = new SqlJoinExpression(foreignMapping.RuntimeType, this._joinOrd++, foreignMapping.DbObjectReference,
            captureName, null);
          join.AddExpression(
            new SqlComparisonExpression(ExpressionType.Equal,
              item,
              new SqlMemberAccessExpression(this.Mapping.QuoteObjectName(foreignCol.TargetName), foreignCol.RuntimeType,
                foreignCol,
    ...
    SqlValueExpression MapExpressionFromJoin(SqlValueExpression inner, Stack<MemberExpression> stack)
    {
      ...
      while (stack.Count > 0)
      {
        ...
        if (this._translations.TryGetValue(it, out res))
        {
          ...
          fromMapping = (item.Type == typeof(TDataModel)) ? this.Mapping : Mappings.AccessMappingFor(item.Type);
        }
        else
        {
          var m = it.Member;
              ...
              || it.Type.IsInterface && Mappings.ExistsFor(it.Type))
          {
            ...
            fromMapping = (it.Type == typeof(TDataModel)) ? this.Mapping : Mappings.AccessMappingFor(it.Type);
          }
          else
          {
            var toCol = fromMapping.Columns.Single(c => c.Member == m);
